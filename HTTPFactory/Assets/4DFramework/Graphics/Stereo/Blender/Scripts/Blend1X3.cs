...
public class Blend1X3 : MonoBehaviour
{
    ...
    RenderTexture GetRenderTexture(Camera cam)
    {
        try
        {
            RenderTexture rt = new RenderTexture(2048, 1024, 24, RenderTextureFormat.Default);
    ...
    {
        yield return null;
        Screen1X3.Instance._canvas.GetComponent<Renderer>().material.mainTexture = GetRenderTexture(_camera);
        if (_UIcamera)
        {
            Screen1X3.Instance._canvas.GetComponent<Renderer>()
                .material.SetTexture("_UITex", GetRenderTexture(_UIcamera));
            Screen1X3.Instance._canvas.GetComponent<Renderer>()
                .material.SetColor("_UIColor", Color.white);
        }

        Screen1X3.Instance.CopyEffect();
    }
